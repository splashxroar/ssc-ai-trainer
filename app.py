...
if selected_subject == "Math (Quant)":
    specific_chapter = st.sidebar.selectbox("Module", ["Mixed (All Chapters)", "Number System", "Percentage", "Ratio & Proportion", "Time & Work", "Time, Speed & Distance", "Algebra", "Geometry", "Trigonometry", "Mensuration", "Data Interpretation"])
elif selected_subject == "GK (Polity, History, etc.)":
    specific_chapter = st.sidebar.selectbox("Module", ["Mixed (All Chapters)", "History", "Polity", "Geography", "Economics", "Physics", "Chemistry", "Biology", "Current Affairs", "Static GK"])
elif selected_subject == "English Comprehension":
    specific_chapter = st.sidebar.selectbox("Module", ["Mixed (All Chapters)", "Reading Comprehension", "Cloze Test", "Synonyms & Antonyms", "Idioms & Phrases", "Spotting Errors", "Active/Passive Voice", "Direct/Indirect Speech"])
elif selected_subject == "General Intelligence (Reasoning)":
    specific_chapter = st.sidebar.selectbox("Module", ["Mixed (All Chapters)", "Coding-Decoding", "Blood Relations", "Syllogism", "Number Series", "Venn Diagrams", "Non-Verbal Reasoning", "Matrix"])

# Removed 100 question option per request
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Assessment", "Typing Test", "Leaderboard", "Review", "Profile"])
...
with tab1:
    if 'current_test' not in st.session_state and 'review_data' not in st.session_state:
        ...
        with col1:
            start_standard = st.button(f"Initialize Assessment ({num_questions} Questions)", use_container_width=True)
        with col2:
            start_weakest = st.button(f"Initialize Remedial Assessment", disabled=not weakest_topic, use_container_width=True)
        ...
        with duel_col1:
            if st.button("Generate Synchronization Code", use_container_width=True):
                st.session_state['hosting_duel'] = True
        with duel_col2:
                ...
                if join_code:
                    with st.spinner("Synchronizing..."):
                        ...
                        if lobby_data.data:
                            ...
                            st.error("Invalid synchronization code.")
        ...
        def generate_ai_test(focus_topic, is_duel=False):
            ...
            with st.spinner(f"Compiling {num_questions} questions for {actual_topic}..."):
                prompt = f"Generate a {num_questions}-question multiple-choice test for SSC CGL level. Subject: {actual_topic}. Difficulty Level: {difficulty}. Return ONLY valid JSON format as a list of dictionaries with keys: 'question', 'options' (list of 4 strings), 'answer' (exact string), and 'explanation'. Seed: {time.time()}."
                try:
                    ...
                    st.error("Compilation failed. Please try again.")
    ...
    if 'current_test' in st.session_state:
        ...
        with col_main:
            st.markdown(f"<h4 style='color: #94a3b8;'>Question {current_idx + 1} of {len(test_questions)}</h4>", unsafe_allow_html=True)
        ...
        with col_palette:
            ...
            if st.button("Submit Assessment", use_container_width=True):
                ...
                for i, q in enumerate(test_questions):
                    ...
                    review_data.append({"question": q['question'], "your_answer": ans if ans else "Omitted", "correct_answer": q['answer'], "status": status, "explanation": q.get('explanation', 'Explanation unavailable.')})
                ...
                del st.session_state['current_test'], st.session_state['q_index'], st.session_state['answers']
                st.rerun()
    ...
    if 'review_data' in st.session_state:
        ...
        for i, data in enumerate(st.session_state['review_data']):
            with st.expander(f"Question {i+1}: {data['status']}"):
...
with tab2:
    ...
    with col_start:
        if st.button("Initialize Typing Module", use_container_width=True):
            st.session_state['typing_start_time'] = time.time()
            st.session_state['typing_active'] = True
            st.rerun()
    with col_new:
        if st.button("Cycle Text Passage", use_container_width=True):
            st.session_state['typing_prompt'] = random.choice(TYPING_PASSAGES)
            if 'typing_start_time' in st.session_state: del st.session_state['typing_start_time']
            st.rerun()
    ...
    if st.session_state.get('typing_active'):
        ...
        if st.button("Conclude & Submit", use_container_width=True):
            ...
            wpm = round((len(typed_text) / 5) / time_spent_min, 1)
...
with tab3:
    ...
    if history_response.data:
        ...
        st.write("#### Aggregated User Statistics")
        grind_stats = df.groupby('username').agg(Assessments_Completed=('score', 'count'), Total_Hours=('time_spent', lambda x: round(x.sum()/60, 2))).reset_index()
        st.dataframe(grind_stats.sort_values(by=['Assessments_Completed'], ascending=[False]), use_container_width=True, hide_index=True)
        st.divider()
        ...
        if not df_ranked.empty:
            ...
            df_ranked['Duration (Min)'] = df_ranked['time_spent'].round(2)
            lb_tabs = st.tabs(["Quantitative", "General Awareness", "Reasoning", "English", "Composite"])
            
            def render_lb(filtered_df):
                if not filtered_df.empty: st.dataframe(filtered_df.sort_values(by=['score', 'Duration (Min)'], ascending=[False, True]).head(20)[['username', 'subject', 'score', 'total', 'Accuracy', 'Duration (Min)']], use_container_width=True, hide_index=True)
                else: st.info("Insufficient data for this classification.")
            ...
            st.write("Standardized criteria not met. Log 25+ question assessments to populate.")
    else:
        st.info("System records are currently empty.")

# --- TAB 4: REVIEW ---
with tab4:
    st.subheader(f"Historical Assessment Data: {current_user}")
    ...
    if past_tests.data:
        for test in past_tests.data:
            time_display = f" | {round(test.get('time_spent', 0), 2)} mins" if test.get('time_spent', 0) else ""
            with st.expander(f"{str(test['created_at'])[:10]} | {test['subject']} | Score: {test['score']}/{test['total']}{time_display}"):
                if test.get('review_data'):
                    for i, q in enumerate(test['review_data']):
                        ...
                        st.info(f"Context: {q.get('explanation', '')}")
                        st.divider()
    else:
        st.write("No historical data available for this user.")
...
with tab5:
    ...
    with col_bio:
        ...
        new_bio = st.text_area("Input background information...", value=player_data.get("bio", ""), height=120, max_chars=300)
